...
extern "C"
{
     ...
     *
     * Resolves the owning eBPF program from the link's binding context under link
     * synchronization and takes an object reference on it. The caller owns the
     * returned reference and must release it with EBPF_OBJECT_RELEASE_REFERENCE.
     * This is the bridge used by base-map providers (e.g. CPUMAP) that are handed
     * an ebpf_link_t* as their client binding context at attach time and need to
     * enumerate the maps associated with the attached program.
     *
