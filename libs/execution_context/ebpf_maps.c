...
typedef struct _ebpf_custom_map
{
    ...
    // outside the map lock. Final cleanup can neither be rejected nor fail, so it must not allocate: if that snapshot
    // had to be allocated during teardown, an allocation failure would delete the element without ever delivering
    // postprocess_map_delete_element, silently leaking the provider's per-entry state exactly when it hurts most.
    // Both pointers are NULL for providers that do not register postprocess_map_delete_element; those keep their
    // notify-before-removal cleanup and never read these buffers.
    ...
    _Outptr_ ebpf_core_map_t** map)
{
    ...
    *map = NULL;
    ...
    // Delete-notification delivery is selected by callback semantics, not by which other callbacks the provider
    // happens to supply.
    //
    ...
    if (HAS_POST_DELETE_ELEMENT_CALLBACK(custom_map->provider_dispatch)) {
        ...
        // afterwards, outside the map lock. Final cleanup cannot be rejected and cannot fail, so it must not allocate:
        // an allocation failure there would delete the element without ever delivering
...
Done:
    ...
    // The dispatch-table copy is cache-aligned, so it must be released with the matching cache-aligned free. Freeing
    // it with the plain allocator mismatches the pool flags.
